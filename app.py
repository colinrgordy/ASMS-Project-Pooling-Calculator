...
with tab1:
    ...
    plate_prefix = st.text_input(
        ...
        help="A unique identifier is required to label tracking tables and output files."
    )
    ...
    with up_col2:
        ...
        uploaded_inventory = st.file_uploader(
            ...
            type=["csv", "xlsx"], 
            help="Provide the manifest file containing real-world freezer locations."
        )
            ...
            try:
                ...
                if exact_mass == 0 or not smiles or smiles.strip() == "":
                    omissions.append({
                        'SDF_Record_Index': idx + 1,
                        'NCGC_ID': sample_id,
                        'Omission_Reason': "Missing SMILES or Zero Weight"
                    })
                    continue
                ...
                omissions.append({
                    'SDF_Record_Index': idx + 1,
                    'NCGC_ID': sample_id,
                    'Omission_Reason': f"Exception: {str(e)}"
                })
                continue
    ...
    def assign_wells_advanced(df, target_size, prefix, vol_comp, target_source_vol_ul, lib_stock_uM, assay_vol, assay_conc):
        ...
        for mode, group in df.groupby('Predicted_Mode'):
            ...
            for pool in stride_pools:
                ...
                min_delta_observed = min(pool_mzs[i+1] - pool_mzs[i] for i in range(len(pool_mzs)-1)) if len(pool_mzs) > 1 else float('inf')
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Calculating library pools..."):
            ...
            if not raw_df.empty:
                ...
                assay_coordinates = [f"{r}{c:02d}" for r in ['A','B','C','D','E','F','G','H'] for c in range(1, 13)]
                ...
                dash_col2.metric("384-Well Source Pools", len(source_map['Source_Well_384'].unique()))
                dash_col3.metric("96-Well Assay Plates", math.ceil(len(source_map[['Assay_Plate_96', 'Assay_Well_96']].drop_duplicates()) / 96))
                dash_col4.metric("DMSO Back-Flush Actions", source_map[source_map['DMSO_Backflush_Volume_nL'] > 0]['Source_Well_384'].nunique())
                ...
                with pd.ExcelWriter(buf_src, engine='openpyxl') as writer:
                    source_map.to_excel(writer, sheet_name="384_Source_Prep", index=False)
                buf_src.seek(0)
                
                down_col1.download_button(
                    label="1. Download Source Prep Manifest (.xlsx)",
                    data=buf_src,
                ...
                echo_picklist_df.columns = ['Source Plate Name', 'Source Well', 'Destination Plate Name', 'Destination Well', 'Transfer Volume']
                
                buf_pick = io.StringIO()
                echo_picklist_df.to_csv(buf_pick, index=False)
                down_col2.download_button(
                    label="2. Download Echo Assay Picklist (.csv)",
                    data=buf_pick.getvalue(),
...
with tab3:
    ...
    if s_map is not None and s_survey is not None:
        try:
            ...
            # Parse stacked blocks in survey file
